...
template sudoku() {
    ...
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            component puzzleRP = RangeProof(252);
            component solutionRP = RangeProof(252);
            puzzleRP.in <== puzzle[i][j];
            puzzleRP.range[0] <== 1;
            puzzleRP.range[1] <== 9;
            assert(puzzleRP.out === 1);

            solutionRP.in <== solution[i][j];
            solutionRP.range[0] <== 1;
            solutionRP.range[1] <== 9;
            assert(solutionRP.out === 1);
